...
VOID open_write_protect(KIRQL irql)
{
...
void vmexit_vmcall_handler(__vcpu* vcpu)
{
	...
	switch (vmcall_reason)  //什么原因调用vmcall
	{
		case VMCALL_TEST:
		{
			uint64_t buffer = 66666;
			PVOID va = (PVOID)vmcall_parameter2;
			cr3 guest_cr3;
			guest_cr3.flags = vmcall_parameter1;

			hv::write_guest_virtual_memory(guest_cr3, va, (PVOID) & buffer, sizeof(uint64_t));
